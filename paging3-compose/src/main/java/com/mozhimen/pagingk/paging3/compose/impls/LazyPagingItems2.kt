...
import com.mozhimen.pagingk.paging3.compose.utils.getPlaceholderList
...
/**
 * @ClassName LazyPagingItems
 * @Description TODO
 * @Author Mozhimen / Kolin Zhao
 * @Date 2024/10/20 15:24
 * @Version 1.0
 */
 ...
 */
public class LazyPagingItems2<T : Any> internal constructor(
    /**
    ...
    var itemSnapshotList by mutableStateOf(pagingDataPresenter.snapshot())
        private set
    var itemSnapshotStateList: SnapshotStateList<T?> = mutableStateListOf() //SnapshotStateList<T?>().also { it.addAll(pagingDataPresenter.snapshot().getPlaceholderList()) }
        private set
    private val _itemSnapshotListStateFlow = MutableStateFlow(itemSnapshotStateList)
    val itemSnapshotStateListFlow: StateFlow<List<T?>> get() = _itemSnapshotListStateFlow
    ...
    private fun updateItemSnapshotList() {
        ...
        if (itemSnapshotStateList.isEmpty()) {
            itemSnapshotStateList = SnapshotStateList<T?>().also { it.addAll(itemSnapshotList.getPlaceholderList()) }
            _itemSnapshotListStateFlow.value = itemSnapshotStateList
        } else {
            val newItemSnapshotList = itemSnapshotList.mapIndexed { index, t -> index to t }.filter { it.second != null }
            newItemSnapshotList.forEach { pair ->
                if (pair.first >= itemSnapshotStateList.size) {
                    itemSnapshotStateList.add(pair.second)
                } else {
                    if (itemSnapshotStateList.get(pair.first) == null) {
                        itemSnapshotStateList.set(pair.first, pair.second)
                    }
                }
            }
        }
    ...
    operator fun set(index: Int, element: T?) {
        if (index in itemSnapshotStateList.indices) {
            UtilKLogWrapper.d(TAG, "set: index $index item ${element}")
            itemSnapshotStateList.set(index, element)
        }
    }
    ...
    operator fun get(index: Int): T? {
        pagingDataPresenter.get(index) // this registers the value load
        return itemSnapshotStateList[index].also { UtilKLogWrapper.d(TAG, "get: index $index item ${it}") }
    }
    ...
    fun peek(index: Int): T? {
        return itemSnapshotStateList[index]
    }
 ...
 */
@Composable
public fun <T : Any> Flow<PagingData<T>>.collectAsLazyPagingItems2(
    context: CoroutineContext = EmptyCoroutineContext
): LazyPagingItems2<T> {

    val lazyPagingItems2 = remember(this) { LazyPagingItems2(this) }

    LaunchedEffect(lazyPagingItems2) {
        if (context == EmptyCoroutineContext) {
            lazyPagingItems2.collectPagingData()
        } else {
            withContext(context) {
                lazyPagingItems2.collectPagingData()
            }
    ...
    LaunchedEffect(lazyPagingItems2) {
        if (context == EmptyCoroutineContext) {
            lazyPagingItems2.collectLoadState()
        } else {
            withContext(context) {
                lazyPagingItems2.collectLoadState()
            }
